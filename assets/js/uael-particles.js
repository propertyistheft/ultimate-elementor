...
function particles_background_script( $scope ) {
	...
	particles_style = $scope.find( '.uael-particle-wrapper' ).data( 'uael-partstyle' );
	...
	if( typeof particles_style != 'undefined' ) {
		if( element_type == 'column' ) {
			...
			$particle_wrap = $scope.find( '#uael-particle-' + id );
			if( $bg_wrap.next( '#uael-particle-' + id ).length == 0 ) {
				$bg_wrap.after( $particle_wrap );
			}
		}
		$selector = $scope.find( '#uael-particle-' + id );

	} else {

		particles_style = $scope.data( 'uael-partstyle' );
	...
	var particles_dot_color = $selector.data( 'uael-partcolor' ),
		particles_custom = $selector.data( 'uael-partdata' ),
		particles_opacity = $selector.data( 'uael-partopacity' ),
		particles_direction = $selector.data( 'uael-partdirection' );

	if( particles_custom !== '' && particles_style == 'custom' ) {
		tsParticles.load( 'uael-particle-' + id, particles_custom );
		...
		if ( $scope.hasClass( 'uael-particle-adv-yes' ) ) {
			var particles_number = $selector.data( 'uael-partnum' );
			var particles_size = $selector.data( 'uael-partsize' );
			var particles_speed = $selector.data( 'uael-partspeed' );
			var particles_interactive = $selector.data( 'uael-interactive' );
